...
> 答： push、replace、go
> push、replace是跳转到新的指定路径页面 push会向路由栈添加路由 而replace不会添加
> go是指跳转回之前的页面 比如跳转到当前页面、跳转到上一页、下一页 在用户注册、跳转到详情页编辑数据完成以后可以通过跳转上一页减少路由的添加
> push和replace都可以携带参数跳转 go只能跳转回之前的页面

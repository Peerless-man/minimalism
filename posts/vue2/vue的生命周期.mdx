...
> 可能还是有很多童鞋会遇到获取不到dom元素的bug的 首先就是先检查一下代码是否写的又问题 其次再F12检查页面上是否有这个元素 然后再检查自己是否是在元素渲染完成后才去获取的这个元素

问： 父子组件的生命周期执行顺序

> 答：组件渲染的顺序是先父后子，渲染完成的顺序是先子后父
> 组件更新的顺序是先父后子，更新完成的顺序是先子后父
> 组件销毁的顺序是先父后子，销毁完成的顺序是先子后父

问：在 created、还是 mounted 里进行异步请求(敲重点)

> 答：如果对页面布局影响不大的异步请求(比如页面布局无论内容大小都是撑满的) 放在 created 还是 mounted 里都无所谓 页面没有组件嵌套的情况下区别也不是很大
